...
int main(){
    ...
    while(true){
        ...
        for(int dir = 0; dir < NUM_DIRECTIONS; dir++){
            if(dir == current_node.last_dir && current_node.last_dir_num == 10){ //must turn
                continue;
            }
            if(current_node.last_dir_num != 0 && (dir - current_node.last_dir == 2 || dir - current_node.last_dir == -2)){  //can't go directly back
                continue;
            }
            int new_last_dir_num = current_node.last_dir_num + 1;
            int num_steps = 1;
            int dx = ((dir - 1) % 2);
            int dy = ((dir - 2) % 2);
            if(dir != current_node.last_dir || current_node.last_dir_num == 0){
                num_steps = 4;
                new_last_dir_num = 4;
            }

            int new_x = current_node.x + dx*num_steps;
            int new_y = current_node.y + dy*num_steps;
            if(new_x < 0 || new_x >= MAP_WIDTH || new_y < 0 || new_y >= MAP_HEIGHT){
                continue;
            }

            int new_total_cost = current_node.total_cost;
            for(int i = 1; i <= num_steps; i++){
                new_total_cost += (map[current_node.x + dx*i][current_node.y + dy*i] - '0');
            }
